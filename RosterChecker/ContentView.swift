...
struct ContentView: View {
    ...
    var fileSelectionView: some View {
        VStack(spacing: 24) {
            ...
            Button(action: { vm.verify() }) {
                Label("开始核对", systemImage: "checkmark.circle")
                    .frame(width: 160)
            }
            .buttonStyle(.borderedProminent)
            .controlSize(.large)
    ...
    func fileCard(title: String, icon: String, url: URL?, action: @escaping () -> Void) -> some View {
        VStack(spacing: 12) {
            Image(systemName: url != nil ? "doc.fill" : icon)
                .font(.system(size: 40))
                .foregroundColor(url != nil ? .blue : .gray)
    ...
    func errorView(_ msg: String) -> some View {
        VStack(spacing: 16) {
            ...
            Button("重试") { vm.state = .idle; vm.result = nil }
    ...
    var resultView: some View {
        VStack(spacing: 0) {
            if let r = vm.result {
                ...
                Group {
                    if vm.selectedTab == 0 {
                        errorsTable(r.errors)
                    } else if vm.selectedTab == 1 {
                        rosterOnlyTable(r.only_roster)
                    } else {
                        clOnlyTable(r.only_cl)
                    }
                }
            }
        }
    ...
    func errorsTable(_ items: [ErrorItem]) -> some View {
        VStack(spacing: 0) {
            tableHeaderRow(["姓名", "部门", "直拨电话(花名册)", "直拨电话(CL)", "手机(花名册)", "手机(CL)"])
            Divider()
            if items.isEmpty {
                ...
                ScrollView {
                    LazyVStack(spacing: 0) {
                        ForEach(items) { item in
                            dataRow([item.name, item.dept, item.dl_excel, item.dl_cl, item.mob_excel, item.mob_cl], highlight: true)
                        }
                    }
                }
            }
    ...
    func clOnlyTable(_ items: [CLOnlyItem]) -> some View {
        VStack(spacing: 0) {
            ...
            Divider()
            if items.isEmpty {
                emptyText("没有 ContactList 独有的人员 ✓")
            } else {
                ScrollView {
                    LazyVStack(spacing: 0) {
                        ForEach(items) { item in
                ...
                ForEach(Array(cols.enumerated()), id: \.offset) { i, val in
                    HStack {
                        Text(val.isEmpty ? "-" : val)
                            .font(.caption)
                            .foregroundColor(highlight && i >= 2 && !val.isEmpty ? .red : .primary)
                        Spacer()
                    }
